...
near 37 percent, and the facts strip, the widest element in the column, ends around 46. The band
...
- `.hero__facts dt` is `#C4C9D0` rather than `--mist`, because that strip runs the full width of
  the copy column, out to where the scrim has already started to release.
...
Above 900px the hero is two columns over the photograph: headline, supporting line, actions and a
four item facts strip on the left, the request card on the right, and a marquee of services along
the bottom edge.
...
The facts strip stays two up down to 360px. Four rows of one costs about 140px of a 748px hero,
which is why it does not stack sooner.
...
The stars are gold, `#EBC001`. The testimonial stars further down the page are `--red`. That is
currently the only place two different star colours appear, so pick one.
